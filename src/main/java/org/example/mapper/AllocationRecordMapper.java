...
public interface AllocationRecordMapper {
        ...
        // 插入分配记录（包含bed_no字段）- 删除重复方法，保留这一个
        @Insert("INSERT INTO AllocationRecord(student_id, room_id, admin_id, allocate_date, status, fee_status, bed_no) "
                        +
                        "VALUES(#{studentId}, #{roomId}, #{adminId}, #{allocateDate}, #{status}, #{feeStatus}, #{bedNo})")
        ...
        // 根据房间ID查询分配记录
        @Select("SELECT * FROM AllocationRecord WHERE room_id = #{roomId}")
        List<AllocationRecord> findByRoomId(@Param("roomId") Integer roomId);

        @Select("SELECT * FROM AllocationRecord WHERE student_id = #{studentId} ORDER BY allocate_date DESC")
        List<AllocationRecord> findByStudentId(@Param("studentId") Integer studentId);
}