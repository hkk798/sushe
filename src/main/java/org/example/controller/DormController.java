...
public class DormController {
    ...
    public String myDorm(HttpSession session, Model model) {
        ...
        try {
            ...
            if (dormInfo != null) {
                // 关键修复：正确处理roomId的类型
                Object roomIdObj = dormInfo.get("roomId");
                Integer roomId = null;
                ...
                if (roomIdObj != null) {
                    if (roomIdObj instanceof Integer) {
                        ...
                        try {
                            ...
                            System.err.println("roomId格式错误: " + roomIdObj);
                        }
                ...
                System.out.println("当前学生ID: " + currentStudent.getStudentId());
                System.out.println("房间ID（转换后）: " + roomId);

                if (roomId != null) {
                    Map<String, Object> otherRoommatesInfo = dormService.getRoommatesInfo(roomId,
                            currentStudent.getStudentId());
                    if (otherRoommatesInfo != null && otherRoommatesInfo.get("roommatesList") != null) {
                        Object roommatesListObj = otherRoommatesInfo.get("roommatesList");
                        if (roommatesListObj instanceof List) {
                            otherRoommates = (List<Map<String, Object>>) roommatesListObj;
                        }
                    }

                    System.out.println("获取到舍友数量: " + otherRoommates.size());
                    if (!otherRoommates.isEmpty()) {
                        System.out.println("第一个舍友: " + otherRoommates.get(0));
                    }
                }
            }

            model.addAttribute("otherRoommates", otherRoommates);

            // 计算入住情况统计
            Map<String, Object> stats = dormService.getDormStats(dormInfo,
                    Map.of("roommatesList", otherRoommates, "totalCount", otherRoommates.size()));
            ...
            System.out.println(
                    "otherRoommates 类型: " + (otherRoommates != null ? otherRoommates.getClass().getName() : "null"));
            System.out.println("otherRoommates 大小: " + (otherRoommates != null ? otherRoommates.size() : 0));

            if (otherRoommates != null && !otherRoommates.isEmpty()) {
                System.out.println("第一个舍友的数据类型: " + otherRoommates.get(0).getClass().getName());
                System.out.println("第一个舍友的键集合: " + otherRoommates.get(0).keySet());

                // 检查每个字段
                Map<String, Object> firstRoommate = otherRoommates.get(0);
                for (String key : firstRoommate.keySet()) {
                    Object value = firstRoommate.get(key);
                    System.out.println("  键: " + key + " = " + value + " (类型: " +
